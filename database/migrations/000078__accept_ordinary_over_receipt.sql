-- FridgeScanner BE-06
-- 000078__accept_ordinary_over_receipt.sql
-- Atomically accept and physically materialize one detected same-Product over-receipt.
...
alter table fridge.purchase_receiving_exception_resolution
  add constraint receiving_exception_resolution_household_identity_uq
  unique (household_id, purchase_receiving_exception_resolution_id);

create table fridge.household_accept_ordinary_over_receipt_command (
  ...
  constraint household_accept_ordinary_over_receipt_command_pk
    primary key (household_id, command_id),
  constraint household_accept_ordinary_over_receipt_command_household_fk
    foreign key (household_id) references fridge.household (household_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_actor_fk
    foreign key (actor_user_id) references fridge.user_profile (user_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_exception_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_detection_fk
    foreign key (
      household_id,
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_conversion_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_location_fk
    foreign key (household_id, storage_location_id)
    references fridge.storage_location (household_id, storage_location_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_compartment_fk
    foreign key (household_id, compartment_id)
    references fridge.compartment (household_id, compartment_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_resolution_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_receipt_item_fk
    foreign key (household_id, result_receipt_item_id)
    references fridge.receipt_item (household_id, receipt_item_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_allocation_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_stock_fk
    foreign key (household_id, result_stock_item_id)
    references fridge.stock_item (household_id, stock_item_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_movement_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_effect_fk
    ...
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_result_unit_fk
    foreign key (result_accepted_excess_unit_id)
    references fridge.measurement_unit (measurement_unit_id)
    on update restrict on delete restrict,
  constraint household_accept_ordinary_over_receipt_command_placement_ck
    check (
  ...
  constraint household_accept_ordinary_over_receipt_command_provenance_ck
    check (btrim(provenance) <> ''),
  constraint household_accept_ordinary_over_receipt_command_outcome_ck
    check (outcome_code in ('PENDING', 'ACCEPTED')),
  constraint household_accept_ordinary_over_receipt_command_result_ck
    check (
...
comment on table fridge.household_accept_ordinary_over_receipt_command is
  'Durable CommandId and same-transaction physical-claim authority for accepting one DETECTED same-Product over-receipt. Candidate identities are result-only; committed replay returns original physical and accepted-excess identities.';

alter table fridge.household_accept_ordinary_over_receipt_command enable row level security;
create policy household_isolation on fridge.household_accept_ordinary_over_receipt_command
  using (household_id = fridge_internal.current_household_id())
  with check (household_id = fridge_internal.current_household_id());

create trigger household_accept_ordinary_over_receipt_command_immutable
before update or delete on fridge.household_accept_ordinary_over_receipt_command
for each row when (old.outcome_code = 'ACCEPTED')
execute function fridge_internal.reject_historical_mutation();

-- Keep DETECTED as a hard workflow barrier for all ordinary/substitution paths.
-- The only exception is an exact PENDING authority created by the dedicated
-- ordinary over-receipt acceptance boundary in the same transaction.
...
comment on function fridge_internal.claim_receipt_item_intent_physical_materialization() is
  'Shared ordinary/substitution physical claim. DETECTED over-receipt remains a barrier except for the exact ReceiptItem candidate authorized by a same-transaction PENDING ACCEPT_ORDINARY_OVER_RECEIPT command.';
...
create or replace function fridge_internal.accept_ordinary_over_receipt(
  p_household_id uuid,
...
begin
  ...
  if found then
    ...
    if v_existing_outcome = 'ACCEPTED' then
      perform fridge_internal.register_household_procurement_command_intent(
        p_household_id, p_command_id, 'ACCEPT_ORDINARY_OVER_RECEIPT'
      );
    ...
    raise exception 'unexpected pending ordinary over-receipt acceptance command';
  end if;
  ...
  -- Re-check replay after the intent lock so a concurrent identical command
  -- cannot degrade into a workflow conflict after the winner commits.
  select c.actor_user_id,
         c.purchase_receiving_exception_id,
         c.placement_kind,
         c.storage_location_id,
  ...
  begin
    select * into v_required_excess
      from fridge_internal.ordinary_receiving_required_accepted_excess(
        p_household_id,
        v_purchase_item_id,
  ...
  insert into fridge.household_accept_ordinary_over_receipt_command (
    ...
    candidate_receipt_item_id, candidate_purchase_item_receipt_allocation_id,
  ...
  insert into fridge.purchase_item_receipt_allocation (
    ...
    v_intent.intended_unit_id, v_conversion_evidence_id, v_provenance, v_committed_at
  );
  ...
  update fridge.household_accept_ordinary_over_receipt_command
...
comment on function fridge_internal.accept_ordinary_over_receipt(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,text,uuid,uuid,uuid,uuid,uuid,uuid) is
  'BE-06 governed ordinary over-receipt acceptance. Atomically revalidates current receiving truth, computes the exact incremental excess, materializes same-Product receipt ingress, and appends ACCEPTED_ORDINARY_EXCESS resolution evidence. PurchaseItem purchased quantity is never changed.';

revoke all on table fridge.household_accept_ordinary_over_receipt_command
  from public, fridge_app, fridge_worker, fridge_readonly;
revoke all on function fridge_internal.accept_ordinary_over_receipt(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,text,uuid,uuid,uuid,uuid,uuid,uuid)
  from public, fridge_worker, fridge_readonly;
grant execute on function fridge_internal.accept_ordinary_over_receipt(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,text,uuid,uuid,uuid,uuid,uuid,uuid)
  to fridge_app;
