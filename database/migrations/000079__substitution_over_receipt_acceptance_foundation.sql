-- FridgeScanner BE-06
-- 000079__substitution_over_receipt_acceptance_foundation.sql
-- Generalize accepted over-receipt resolution/conservation to exact substitution allocations.

begin;

alter table fridge.purchase_item_substitution_allocation
  add constraint substitution_allocation_resolution_identity_uq
  unique (
    household_id,
    purchase_item_id,
    receipt_item_id,
...
alter table fridge.receipt_item_intent_substitution_materialization
  add constraint receipt_item_intent_substitution_materialization_resolution_identity_uq
  unique (
    household_id,
...
alter table fridge.purchase_receiving_exception_resolution
  alter column ordinary_allocation_id drop not null,
  add column substitution_allocation_id uuid;
...
alter table fridge.purchase_receiving_exception_resolution
  ...
  add constraint receiving_exception_resolution_substitution_allocation_fk
    foreign key (
      household_id,
      purchase_item_id,
      receipt_item_id,
  ...
  add constraint receiving_exception_resolution_substitution_allocation_uq
    unique (household_id, substitution_allocation_id),
  add constraint receiving_exception_resolution_exact_substitution_materialization_fk
    foreign key (
      household_id,
      receipt_item_intent_id,
      receipt_item_id,
      substitution_allocation_id
    ) references fridge.receipt_item_intent_substitution_materialization (
      household_id,
      receipt_item_intent_id,
      receipt_item_id,
      purchase_item_substitution_allocation_id
...
comment on table fridge.purchase_receiving_exception_resolution is
  'Append-only BE-06 accepted over-receipt evidence. Exactly one ordinary or substitution allocation is bound to the exact detected ReceiptItemIntent materialization and exact accepted excess portion; PurchaseItem purchased quantity never changes.';

comment on constraint receiving_exception_resolution_exact_substitution_materialization_fk
...
comment on function fridge_internal.receiving_required_accepted_excess(uuid,uuid,numeric,numeric,uuid,uuid) is
  'Locks one PurchaseItem and returns the exact incremental portion of one proposed ordinary or substitution allocation outside remaining purchased allowance after all previously accepted allocation-local excess portions.';
...
set search_path = pg_catalog, fridge, fridge_internal
as $$
  select * from fridge_internal.receiving_required_accepted_excess(
    p_household_id,
    p_purchase_item_id,
    p_quantity_num,
    p_quantity_den,
...
comment on function fridge_internal.assert_purchase_receiving_pool(uuid,uuid) is
  'Serializes on PurchaseItem and proves ordinary plus substitution covered quantity does not exceed purchased quantity. Exact allocation-local portions backed by accepted over-receipt resolution evidence are excluded without modifying PurchaseItem truth.';
