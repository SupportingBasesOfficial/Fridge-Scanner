...
comment on table fridge.measurement_conversion_rule is
  'Versioned permission/profile for unit conversion. EXACT_FACTOR stores its canonical factor. CONTEXTUAL_FACTOR identifies a typed/versioned context contract; every committed use stores the exact factor and exact named inputs selected in MeasurementConversionEvidence.';
...
comment on table fridge.measurement_conversion_evidence is
  'Immutable exact conversion decision. The committed target must equal source × applied factor in rational arithmetic; contextual formula inputs are stored as typed child rows; presentation rounding never participates.';
...
create table fridge.measurement_conversion_evidence_input (
  measurement_conversion_evidence_input_id uuid primary key,
  measurement_conversion_evidence_id uuid not null,
  input_name text not null,
  input_quantity_num numeric not null,
  input_quantity_den numeric not null,
  measurement_unit_id uuid,
  provenance text,
  constraint conversion_evidence_input_evidence_fk
    foreign key (measurement_conversion_evidence_id)
    references fridge.measurement_conversion_evidence (measurement_conversion_evidence_id)
    on update restrict on delete restrict,
  constraint conversion_evidence_input_unit_fk
    foreign key (measurement_unit_id)
    references fridge.measurement_unit (measurement_unit_id)
    on update restrict on delete restrict,
  constraint conversion_evidence_input_name_nonblank
    check (btrim(input_name) <> ''),
  constraint conversion_evidence_input_quantity_normalized
    check (fridge_internal.assert_normalized_rational(input_quantity_num, input_quantity_den)),
  constraint conversion_evidence_input_name_uq
    unique (measurement_conversion_evidence_id, input_name)
);

comment on table fridge.measurement_conversion_evidence_input is
  'Named exact rational input used by a contextual conversion contract. The governed commit routine validates the required input set against the referenced contract code/version.';
...
comment on table fridge.currency is
  'Governed ISO-style alpha-3 currency identity. Authoritative monetary facts always reference an explicit currency.';
...
create table fridge.money_rounding_policy (
  ...
  constraint money_rounding_policy_scale_valid check (decimal_scale >= 0),
