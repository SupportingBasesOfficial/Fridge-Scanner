...
begin
  if not exists (
    ...
    raise exception 'canonical Household catalog administration capability is missing';
  end if;
  ...
  if exists (
    ...
    raise exception 'BE-05 must not guess concrete Household role-to-catalog-capability mappings';
  end if;
  ...
  if has_table_privilege('fridge_app', 'fridge.household_capability', 'SELECT')
    ...
    raise exception 'fridge_app must not receive direct Household catalog authority reference-table access';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'fridge_app must be able to acquire governed Household catalog administration authority';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception 'worker/readonly capabilities must not materialize catalog administration authority';
  end if;
  ...
  if position('statement_timestamp()' in v_definition) > 0 then
    raise exception 'catalog administration authority must not use pre-serialization statement_timestamp()';
  end if;

  if position('HOUSEHOLD_CATALOG_ADMINISTER' in v_definition) = 0 then
    raise exception 'catalog administration authority function must revalidate the canonical capability';
  end if;
end;
$$;
