...
export interface ProcurementReceivingRouteDependencies {
  readonly listHouseholdPurchases: UseCase<
    ListHouseholdPurchasesInput,
    ListHouseholdPurchasesOutput
  >;
  readonly getHouseholdPurchase: UseCase<
    GetHouseholdPurchaseInput,
    { readonly purchase: HouseholdPurchaseObservation }
  >;
  ...
  readonly createReceiptItemIntent: UseCase<
    CreateReceiptItemIntentInput,
    CreateReceiptItemIntentOutput
  >;
  readonly materializeOrdinaryReceiptItem: UseCase<
    MaterializeOrdinaryReceiptItemInput,
    MaterializeOrdinaryReceiptItemOutput
  >;
}
...
function parsePurchaseId(value: unknown) {
  ...
  try {
    ...
    throw new InvalidInputError('Purchase identifier is invalid', error);
  }
}

function parseOptionalPurchaseId(value: unknown) {
  if (value === undefined || value === null) return undefined;
  return parsePurchaseId(value);
}
...
function parseReceiptId(value: unknown) {
  ...
  try {
    ...
    throw new InvalidInputError('Receipt identifier is invalid', error);
  }
}
...
function parseReceiptItemIntentId(value: unknown) {
  ...
  try {
    return ReceiptItemIntentId(value);
  } catch (error) {
    throw new InvalidInputError('Receipt item intent identifier is invalid', error);
  }
}
...
function parsePurchaseItemId(value: unknown) {
  ...
  try {
    return PurchaseItemId(value);
  } catch (error) {
    throw new InvalidInputError('Purchase item identifier is invalid', error);
  }
}
...
function parseMeasurementUnitId(value: unknown) {
  ...
  try {
    return MeasurementUnitId(value);
  } catch (error) {
    throw new InvalidInputError('Measurement unit identifier is invalid', error);
  }
}
...
function parseOptionalConversionEvidenceId(value: unknown) {
  ...
  try {
    return MeasurementConversionEvidenceId(value);
  } catch (error) {
    throw new InvalidInputError('Measurement conversion evidence identifier is invalid', error);
  }
}

function parseExactQuantity(value: unknown) {
  if (typeof value !== 'object' || value === null) {
    throw new InvalidInputError('Quantity is invalid');
  }
  ...
  try {
    ...
    throw new InvalidInputError('Quantity is invalid', error);
  }
}
...
function parsePageSize(value: unknown): number | undefined {
  ...
  if (typeof value !== 'string' || !/^\d+$/.test(value)) {
    throw new InvalidInputError('pageSize is invalid');
  }
  return Number(value);
}
...
function parseCursor(occurredAt: unknown, purchaseId: unknown): HouseholdPurchasePageCursor | null {
  ...
  if (typeof occurredAt !== 'string' || typeof purchaseId !== 'string') {
    throw new InvalidInputError('Purchase page cursor is invalid');
  }
  try {
    return { occurredAt: instant(occurredAt), purchaseId: PurchaseId(purchaseId) };
  } catch (error) {
    throw new InvalidInputError('Purchase page cursor is invalid', error);
  }
}
...
function serializePurchase(purchase: HouseholdPurchaseObservation) {
  return {
    ...
    items: purchase.items.map((item) => ({
      ...
      quantity: {
        numerator: item.quantity.numerator.toString(),
        denominator: item.quantity.denominator.toString(),
      },
...
function parsePlacement(value: unknown): MaterializeOrdinaryReceiptItemInput['placement'] {
  if (typeof value !== 'object' || value === null) {
    throw new InvalidInputError('Receipt placement is invalid');
  }
  ...
  if (kind === 'LOCATION') {
    ...
    try {
      return { kind: 'LOCATION', storageLocationId: StorageLocationId(id) };
    } catch (error) {
      throw new InvalidInputError('Storage location placement is invalid', error);
    }
  }
  if (kind === 'COMPARTMENT') {
    ...
    try {
      return { kind: 'COMPARTMENT', compartmentId: CompartmentId(id) };
    } catch (error) {
      throw new InvalidInputError('Compartment placement is invalid', error);
    }
  }
...
export function registerProcurementReceivingRoutes(
  ...
  server.post<{
    ...
    Body: {
      commandId?: unknown;
      transactionCurrencyCode?: unknown;
      items?: unknown;
    };
    ...
    return reply.code(201).send({
      purchaseId: String(output.purchaseId),
      purchaseItemIds: output.purchaseItemIds.map(String),
    });
  });
  ...
  server.post<{
    ...
    const output = await dependencies.createReceipt.execute({
      ...
      actorPrincipalId,
      householdId,
      purchaseId: parseOptionalPurchaseId(request.body?.purchaseId),
  ...
  server.post<{
    ...
    Body: {
      ...
      allocationConversionEvidenceId?: unknown;
      placement?: unknown;
      provenance?: unknown;
    };
  }>(
    ...
    async (request, reply) => {
      ...
      const output = await dependencies.materializeOrdinaryReceiptItem.execute({
        ...
        allocationConversionEvidenceId: parseOptionalConversionEvidenceId(
          request.body?.allocationConversionEvidenceId,
        ),
