...
For the canonical ordinary line equation, `line_gross` is the pre-discount, pre-tax/line-charge extended amount. When unit/basis price exists, unrounded extended gross equals `basis-price amount × (purchased quantity converted exactly into the pricing-basis unit ÷ pricing-basis quantity)`; the preserved currency scale/rounding policy is applied once at the line-gross boundary. The authoritative line gross must reconcile to that result. A differing source gross remains only as an explicit pricing discrepancy/exception with source/computed amounts, conversion evidence, policy, reason and resolution status and cannot silently define ordinary gross or historical unit cost; `line_discount` is the total discount allocated to that line; `line_tax`/governed line charges are the total taxes/charges allocated to that line; and `line_net = line_gross - line_discount + line_tax/line-charges` under the transaction's governed rounding/scale policy. If both source-provided and derived totals are available, they must reconcile under that rounding policy or the discrepancy remains explicit rather than one value silently replacing the other.
...
## D-011 — Idempotency identity is tenant-, principal- and request-bound

**Decision:** an IdempotencyRecord is identified by target Household or explicit non-Household system scope, authenticated actor/trusted principal, operation/command identity and client idempotency key. The key alone is never globally authoritative. Every record preserves a canonical fingerprint of the complete normalized semantic mutation request, including target entity/scope and command version, together with execution state and the committed result/response reference.

A retry must first pass current authorization for the target scope. If scoped identity and fingerprint match, it reuses the original in-progress or committed outcome without executing the mutation again. If the same scoped identity is presented with a different fingerprint, target, operation or version, the command is rejected as an idempotency conflict; the existing record is immutable for that identity and must not be overwritten or reinterpreted. Records belonging to another Household or principal cannot suppress execution or disclose results. Any retention/expiry and later key-reuse policy must prevent overlap with an outcome that remains replayable.

## D-012 — Catalog concepts and compatibility mappings are explicitly scoped

**Decision:** IngredientConcept and Product-to-IngredientConcept compatibility mappings use explicit global-or-Household catalog governance. A global concept/mapping has no Household owner and cannot be mutated by ordinary Household authority. A Household concept/mapping belongs to exactly one Household with visibility/edit authority constrained to it. Global mappings connect only global Products and concepts; Household mappings may connect global or same-Household entities, never another Household's private entity. Global recipes/rules consume only global concepts/mappings; Household consumers may use global or same-Household data after authorization. Cross-scope publication/cloning creates a governed destination version, validates every reference and preserves provenance without silently widening visibility.

## D-013 — Shelf-life state follows quantity lineage

**Decision:** every quantity portion produced by split, transfer or lineage-preserving redistribution inherits or immutably references the SourceExpirationFact, FoodLifecycleEvent and activated ShelfLifeRule evaluation context applicable to its source at operation occurrence time, preserving original occurrence/evaluation anchors, rule/timezone versions and provenance. New destination facts may further restrict expiration but redistribution cannot reset lifecycle state or discard an earlier candidate. Merges require shelf-life/lifecycle-equivalent state or preserved distinguishable sublineages; any aggregate operational expiration is no later than the earliest included lineage candidate, and later splits must recover each portion's fact set deterministically.

## D-014 — ProductIdentifier normalization is governed and versioned

**Decision:** every identifier retains its exact source value, scheme and issuer/namespace plus the governed scheme-specific normalization rule identity/version and normalized result. The rule explicitly defines case, separator/punctuation, whitespace, check-digit, leading-zero, character and validation semantics. Unknown/invalid inputs remain unresolved evidence. Uniqueness is version-aware within scheme/issuer namespace. A rule change creates a new version and explicit migration candidates without mutating history; changed matches/collisions are detected before activation and remain staged for governed merge, alias or rejection. Lookup and active-version selection cannot depend on deployment order.

## D-015 — ShelfLifeRule governance and concept applicability are historical facts

**Decision:** every ShelfLifeRule has explicit global-or-Household catalog scope, ownership, visibility and edit authority. A global rule has no Household owner and references only global catalog entities/predicates. A Household rule belongs to exactly one Household and may reference global or same-Household entities, never another Household's private data. A StockItem may use only global rules or rules owned by its Household. Cross-scope publication/cloning creates a governed destination version, validates references and preserves provenance rather than mutating scope.

When a rule targets IngredientConcept, applicability to a concrete StockItem Product is evaluated at the authoritative rule-group activation anchor using a visible, effective compatibility mapping. The resulting candidate preserves immutable CompatibilityDecisionEvidence with Product, concept, mapping/rule identity and version, evaluation/effective context and provenance. Recalculation reuses that evidence; current compatibility cannot retroactively add or remove an activated deadline. Later mapping changes govern future activations or explicit corrections only. Split/transfer lineage carries this evidence with the activated rule context.

## Consequence for DB-01

DB-01 may now model the relational entities and cardinalities using these decisions as constraints. It must preserve immutable historical evidence for placement-changing transfers, recipe execution snapshots, compatibility decisions, contextual conversions, exact-rational quantity conversion/conservation, relative shelf-life arithmetic including month/year rollover, date-only timezone selection anchors, purchase monetary-role and basis-price-to-line-gross reconciliation, separated receipt-versus-shopping allocation pools, Recipe/RecipeVersion plus IngredientConcept/compatibility catalog governance, tenant/principal/request-bound idempotency, lineage-preserved shelf-life state, governed/versioned ProductIdentifier normalization, explicitly scoped ShelfLifeRule governance and activation-time compatibility evidence for concept-targeted rules. It must not reopen these choices implicitly through table shape; any conflict must be raised explicitly as a governed decision.
