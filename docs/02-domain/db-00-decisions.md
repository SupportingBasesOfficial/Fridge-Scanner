...
The materialized value must be recomputable and must retain provenance sufficient to explain the candidate set, semantic groups, group-local precedence decisions, selected rule versions, evaluation anchors, the SourceExpirationFact occurrence/source anchor used to select date-only timezone context, the selected timezone version/context and final combination result. Changing an authoritative input must invalidate/recalculate the projection deterministically.
