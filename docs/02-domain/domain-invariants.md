...
8. Product is the canonical stockable food/product identity and is not synonymous with a retail SKU. It may represent commercial, loose/unbranded, household-defined or prepared food; commercial identifiers, brand, manufacturer and Batch metadata are optional. Product identity is independent of household stock, purchase price and storage location.
...
17. Stock-changing commands must define atomic concurrency behavior so two valid concurrent actions cannot silently produce an invalid balance. Inventory reconciliation must use the authoritative observation/as-of point captured for each InventoryCountItem and concurrency semantics that preserve all movements committed after that line's cutoff rather than overwriting or double-accounting for them. A session-level cutoff may substitute for per-line cutoffs only when a genuinely atomic/frozen snapshot or equivalent snapshot token guarantees all count lines correspond to the same authoritative state.
...
29. Effective expiration of a concrete StockItem must be explainable from source expiration and applicable lifecycle/storage rules. Every ShelfLifeRule must have explicit governed applicability and version/effective-interval semantics, and rule selection must be deterministic: more specific scope outranks broader scope, then explicit priority resolves ordering. Version/effective-interval selection is evaluated as of the domain occurrence time of the authoritative fact that activates the rule: triggering lifecycle-event occurrence for event-driven rules; authoritative stock-entry occurrence for stock-entry/default rules; authoritative InventoryMovement/InventoryTransfer or other canonical placement-state occurrence for placement/storage-change rules; and trusted storage/conservation observation occurrence for observation-triggered rules. Recalculation must reuse that same original evaluation anchor rather than current/recalculation time. Equally specific conflicting rules with the same effective priority at that evaluation time must be rejected or surfaced for governance rather than chosen arbitrarily. Applicable source and rule-derived deadlines form a candidate set; unless a future explicitly governed semantic-class rule defines otherwise, the effective operational expiration is the earliest applicable candidate, so no later candidate may extend an earlier authoritative deadline. A source expiration expressed only as a calendar date preserves date-only precision as source truth but, for operational ordering, is interpreted as the end of that local calendar day in the canonical Household timezone applicable to the StockItem, unless the source itself supplies an explicit timezone/offset or governed source context. The timezone/context used for this interpretation must be versioned/as-of so later configuration changes cannot alter historical recomputation. Instant candidates retain exact instant semantics; operational normalization must not mutate the original source precision. Any storage/conservation fact allowed to influence shelf-life must come from canonical placement/state or a trusted observation with occurrence time and provenance; uncertain sensor/heuristic evidence must not silently rewrite authoritative shelf-life inputs. EffectiveExpiration must retain provenance to the candidate set, source expiration fact(s), evaluation anchor, selected ShelfLifeRule version(s), storage/conservation inputs, date-only interpretation timezone/context and combination result.
...
33. Domain occurrence time and system recording time are distinct where delayed/offline recording is possible. Physical inventory counts and any other delayed reconciliation workflow must preserve the domain occurrence/as-of point required to evaluate historical state correctly; for non-atomic counting sessions this requirement applies at InventoryCountItem granularity.
