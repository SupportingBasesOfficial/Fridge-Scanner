...
A canonical ProductIdentifier in a scheme that defines one global namespace may belong only to a global Product. A Household-scoped Product must never claim, reserve or consume a globally namespaced canonical key. If a Household scan/import observes a globally namespaced value before canonical global Product resolution, the observation may be retained as Household-scoped staged identifier evidence/claim with source value, normalization evidence, Household provenance and any private Product candidate, but that staged evidence is not a canonical ProductIdentifier and does not participate in global uniqueness or resolution. It becomes canonical only through a governed resolution/promotion workflow that links the global key to a global Product; the workflow must preserve provenance and must not silently widen visibility of the private Product.
...
Every destination quantity created by split, transfer or other lineage-preserving redistribution inherits or immutably references all SourceExpirationFact, FoodLifecycleEvent and already-activated ShelfLifeRule evaluation context — including any concept-applicability CompatibilityDecisionEvidence — applicable to that source quantity at the operation occurrence time, retaining original fact identity, occurrence/evaluation anchors, rule/timezone versions and provenance. The destination may acquire additional placement/storage/lifecycle candidates after the operation, but redistribution cannot reset an opened/prepared state, discard an earlier deadline or treat inherited quantity as fresh. A partial operation associates the inherited state with exactly the transferred portion while the remainder retains it on the source lineage.
...
Each InventoryCountItem records its authoritative physical observation time and the corresponding ledger as-of/cutoff used for that line's reconciliation. When the observation source can provide a trustworthy causal ordering discriminator for facts sharing the same timestamp, such as an authoritative sequence/order token tied to the same clock/stream, that discriminator is preserved with the line. A session-level observation time/cutoff may be reused by all lines only when the workflow provides a genuinely atomic/frozen snapshot or equivalent snapshot token that guarantees all observations correspond to the same authoritative inventory state.

Reconciliation compares each observed line with system state as of that line's captured cutoff and observation ordering, not whatever balance happens to exist when processing occurs later. A movement recorded after the captured cutoff is classified by authoritative domain occurrence ordering, not merely by commit time and not by timestamp equality alone. If its occurrence time is strictly after the physical observation, it is a genuinely intervening/post-observation movement and must be preserved outside the count adjustment. If its occurrence time is strictly before the physical observation, it changes the historical state that the observation should have been compared against: the prior reconciliation basis is invalidated/rebased and must be recomputed with that late fact included before an adjustment can remain or become committed. If movement and observation carry equal occurrence timestamps, a preserved trustworthy causal discriminator applicable to the same ordering domain may classify the movement as pre- or post-observation. Without such causal evidence, equality is ambiguous: the reconciliation must remain unresolved/staged or block/escalate and must neither guess a pre-observation rebase nor preserve the movement as post-observation. A late movement proven to be pre-observation must never be preserved on top of an adjustment that already compensated for its physical effect, because that would double-apply the change.

If a reconciliation adjustment was already committed when a newly recorded pre-observation fact becomes authoritative, the system must not mutate history. It must deterministically recompute the count outcome against the corrected historical state and emit an explicit compensating/reconciliation outcome when safe, or block/escalate when provenance, ordering or required history is insufficient. If a newly recorded fact ties the observation timestamp and lacks trustworthy causal ordering, the committed history remains immutable and the case stays blocked/escalated until ordering is resolved; the system must not invent a compensating outcome from an ambiguous sequence. If the captured historical state can no longer be reconstructed safely, the outcome must be blocked/escalated rather than guessed.

### InventoryCountItem
Represents one observed count line. It must identify the counted Product, observed quantity and MeasurementUnit, plus the observed placement when placement is part of the counting context, and its own authoritative observation time plus ledger as-of/cutoff unless the session proves one common atomic snapshot as defined above. It also preserves any trustworthy causal ordering discriminator supplied by the authoritative observation source when that discriminator is required to order equal-time facts. It may reference an existing StockItem when the observed stock can be matched unambiguously; that reference is optional because physical counting must also represent newly discovered stock that has no prior StockItem.
...
Every committed reconciliation outcome links back to the InventoryCount/InventoryCountItem, that line's authoritative observation/as-of point, the historical movement set/basis used, the deterministic temporal/causal ordering and allocation/match decision, and the committed adjustment or compensating movement(s) it produced.
...
Every ShelfLifeRule has both explicit catalog governance and explicit applicability. A global rule has no Household owner, is editable only through global governance and may reference only global Products, IngredientConcepts, mappings/classifications and other globally visible predicates. A Household rule belongs to exactly one Household, is visible/editable only through that Household's authority and may reference global or same-Household catalog entities, never another Household's private data. A StockItem may use only a global rule or a rule owned by its Household. Promotion, sharing or cloning across scopes creates a governed destination rule/version with validated destination-scope references and preserved provenance; it never silently changes ownership or visibility.

Rules may target a specific Product, an IngredientConcept, or another governed classification introduced later; broader scopes must not override a more specific applicable rule accidentally. Applicability includes a semantic trigger/deadline group identifying which rules are alternatives competing to produce the same kind of deadline for the same authoritative activation fact, plus trigger/event type, storage condition/category and other explicit predicates required by the rule. For an IngredientConcept-targeted rule, deciding that a concrete StockItem Product satisfies the concept is itself a governed compatibility decision evaluated at the rule group's activation anchor. The candidate/activation record preserves or immutably references the exact CompatibilityDecisionEvidence — Product, concept, mapping/rule identity and version, effective/evaluation context and provenance — used then. Recalculation reuses that preserved decision and cannot consult the current mapping to retroactively add/remove the activated candidate; a later compatibility change applies only to future activations or an explicit correction workflow.
...
User ──< HouseholdMembership >── Household
                                ...
                                │       └── per-line observation/as-of + causal-order evidence when needed ──> reconciliation adjustment InventoryMovement
                                │                              ├── optional ──> StockItem
...
GLOBAL Product ──< globally-namespaced canonical ProductIdentifier
Household-private Product ── staged global-identifier evidence/claim only until governed global resolution
...
ShelfLifeRule [GLOBAL or Household] ── group-local precedence / activation-time selection / explicit temporal arithmetic ──> same-scope-visible Product | IngredientConcept | governed classification
Concept-targeted ShelfLifeRule activation ── CompatibilityDecisionEvidence at activation anchor ──> Product / IngredientConcept mapping version
...
- a Household-private Product claiming/reserving a globally namespaced canonical identifier such as GTIN instead of retaining only staged Household-scoped evidence until governed global resolution;
...
- treating equal movement/observation timestamps as causal proof of pre- or post-observation ordering when no trustworthy ordering discriminator exists;
...
- ShelfLifeRule without explicit global-or-Household ownership/visibility/edit authority or with cross-scope private catalog references;
- concept-targeted ShelfLifeRule applicability recomputed from current compatibility instead of preserving activation-time CompatibilityDecisionEvidence;
