...
export class PgHouseholdIngredientConceptMetadataChanger
  implements HouseholdIngredientConceptMetadataChanger
{
  async changeHouseholdIngredientConceptMetadata(
    ...
    try {
      result = await client.query<{
        ...
        `select outcome_code,
           ...
           from fridge_internal.change_household_ingredient_concept_metadata(
    ...
    switch (outcome?.outcome_code) {
      case 'CHANGED':
        if (outcome.result_ingredient_concept_id === null) {
          throw new InternalApplicationError(
            new Error('ChangeHouseholdIngredientConceptMetadata succeeded without identity'),
          );
        }
      ...
      default:
        throw new InternalApplicationError(
          new Error('unexpected ChangeHouseholdIngredientConceptMetadata outcome'),
        );
    }
  }
}
