...
export class PgHouseholdIngredientConceptRetirer implements HouseholdIngredientConceptRetirer {
  async retireHouseholdIngredientConcept(
    ...
    try {
      result = await client.query<{
        ...
        `select outcome_code,
           ...
           from fridge_internal.retire_household_ingredient_concept(
    ...
    switch (outcome?.outcome_code) {
      case 'RETIRED':
        if (outcome.result_ingredient_concept_id === null) {
          throw new InternalApplicationError(
            new Error('RetireHouseholdIngredientConcept succeeded without identity'),
          );
        }
        return IngredientConceptId(outcome.result_ingredient_concept_id);
      case 'DEPENDENCY_CONFLICT':
        throw new ConflictError('IngredientConcept has current compatibility dependencies');
      case 'NOT_FOUND':
      ...
      default:
        throw new InternalApplicationError(
          new Error('unexpected RetireHouseholdIngredientConcept outcome'),
        );
    }
  }
}
