...
function requireExactExternalIdentityComponent(
  ...
  if (value.length === 0 || value.trim().length === 0) throw new TypeError(`${label} must not be blank`);
  if (value !== value.trim()) throw new TypeError(`${label} must not contain surrounding whitespace`);
  if (value.length > maxLength) throw new TypeError(`${label} exceeds maximum length`);
  return value;
}
...
export class HouseholdAuthorizationError extends HouseholdUnauthorizedError {
  constructor() { super(); this.name = 'HouseholdAuthorizationError'; }
}

export class ExternalIdentityMappingError extends Error {
  constructor() { super('external identity mapping failed'); this.name = 'ExternalIdentityMappingError'; }
}
...
export class PgDatabase implements TransactionManager, ReadinessProbe {
  ...
  constructor(options: PgDatabaseOptions) {
    ...
    this.#pool = new Pool({ connectionString: options.connectionString, max: options.maxConnections ?? 10, allowExitOnIdle: false });
  }

  async resolvePrincipalForExternalIdentity(authority: string, subject: string): Promise<PrincipalId | null> {
    if (this.#capabilityRole !== 'fridge_app') {
      throw new TypeError('external identity resolution requires fridge_app capability');
    }
    const exactAuthority = requireExactExternalIdentityComponent(authority, 'external identity authority', EXTERNAL_AUTHORITY_MAX_LENGTH);
    const exactSubject = requireExactExternalIdentityComponent(subject, 'external identity subject', EXTERNAL_SUBJECT_MAX_LENGTH);
    ...
    try {
      ...
      if (client !== null && transactionStarted) {
        try { await client.query('rollback'); } catch { /* preserve dependency failure */ }
      }
  ...
  async withAuthorizedHouseholdTransaction<T>(principalId: PrincipalId, householdId: HouseholdId, operation: (transaction: TransactionHandle) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      await client.query("select set_config('fridge.household_id', $1, true)", [verifiedHouseholdId]);
      const authorization = await client.query<{ membership_id: string; role_code: string }>(
        `select membership_id::text, role_code from fridge.household_membership
          where household_id = $1::uuid and user_id = $2::uuid and lifecycle_status = 'ACTIVE'
            and effective_from <= statement_timestamp()
            and (effective_to is null or effective_to > statement_timestamp())
          order by effective_from desc, membership_id limit 1`,
        [verifiedHouseholdId, verifiedPrincipalId],
      );
      const authorizationRow = authorization.rows[0];
      if (authorizationRow === undefined) throw new HouseholdAuthorizationError();
      const verifiedTransaction = new PgTransactionHandle(
        client,
        verifiedPrincipalId,
        verifiedHouseholdId,
        HouseholdMembershipId(authorizationRow.membership_id),
  ...
  async check(): Promise<ReadinessResult> {
    ...
    if (client === null) return { ready: false, reason: 'database_unavailable' };
    let transactionStarted = false;
    try {
      await client.query('begin');
      transactionStarted = true;
...
export class PgExternalIdentityPrincipalMapper {
  ...
  async resolve(identity: ExternalIdentityKey): Promise<PrincipalId> {
    const principal = await this.database.resolvePrincipalForExternalIdentity(identity.authority, identity.subject);
    if (principal === null) throw new ExternalIdentityMappingError();
    return principal;
  }
}
...
export class PgHouseholdProfileReader implements HouseholdProfileReader {
  async readDisplayName(transaction: TransactionHandle): Promise<string | null> {
    ...
    const result = await client.query<{ display_name: string }>(
      `select display_name from fridge.household where household_id = $1::uuid`,
...
export function requirePgClient(transaction: TransactionHandle): PoolClient {
  if (!(transaction instanceof PgTransactionHandle)) throw new TypeError('transaction handle was not created by PgDatabase');
