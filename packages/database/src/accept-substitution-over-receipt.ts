...
export class PgHouseholdSubstitutionOverReceiptAcceptor implements HouseholdSubstitutionOverReceiptAcceptor {
  async acceptSubstitutionOverReceipt(
    ...
    try {
      const result = await client.query<AcceptanceRow>(
        `select outcome_code,
           ...
           from fridge_internal.accept_substitution_over_receipt(
             $1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid,
             $6::text, $7::text, $8::uuid, $9::uuid, $10::text,
             $11::uuid, $12::uuid, $13::uuid, $14::uuid, $15::uuid, $16::uuid
           )`,
        [
    ...
    switch (outcome?.outcome_code) {
      case 'ACCEPTED':
        if (
          ...
          throw new InternalApplicationError(
            new Error('substitution over-receipt acceptance succeeded without complete result'),
          );
        }
        return {
      ...
      case 'INVALID_INPUT':
        throw new InvalidInputError('substitution over-receipt acceptance input is invalid');
      case 'NOT_FOUND':
        throw new NotFoundError();
      case 'CONFLICT':
        throw new ConflictError('detected over-receipt cannot be accepted through the substitution path');
      case 'NO_OVER_RECEIPT':
        throw new ConflictError('detected exception no longer requires substitution excess acceptance');
      case 'IDEMPOTENCY_CONFLICT':
        throw new IdempotencyConflictError();
      case 'UNAUTHORIZED':
        throw new HouseholdAuthorizationError();
      default:
        throw new InternalApplicationError(new Error('unexpected substitution over-receipt acceptance outcome'));
    }
  }
}
