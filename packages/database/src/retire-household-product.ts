...
export class PgHouseholdProductRetirer implements HouseholdProductRetirer {
  async retireHouseholdProduct(
    ...
    try {
      result = await client.query<{
        ...
        `select outcome_code,
           ...
           from fridge_internal.retire_household_product(
             $1::uuid,
             $2::uuid,
             $3::uuid,
             $4::uuid,
    ...
    switch (outcome?.outcome_code) {
      case 'RETIRED':
        if (outcome.result_product_id === null) {
          throw new InternalApplicationError(
            new Error('RetireHouseholdProduct succeeded without Product identity'),
          );
        }
        return ProductId(outcome.result_product_id);
      case 'DEPENDENCY_CONFLICT':
        throw new ConflictError('Product has current dependencies');
      case 'NOT_FOUND':
      ...
      default:
        throw new InternalApplicationError(
          new Error('unexpected RetireHouseholdProduct outcome'),
        );
    }
  }
}
